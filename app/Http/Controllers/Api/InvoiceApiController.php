...
class InvoiceApiController extends Controller
{
    public function store(StoreInvoiceRequest $request, ICVService $icv, TaxCalculationService $tax): JsonResponse
    {
        ...
        $invoice = DB::transaction(function () use ($data, $company, $icv, $tax) {
            ...
            $inv = Invoice::create(array_merge($totals, ['uuid' => (string) Str::uuid(), 'invoice_number' => $data['invoice_number'], 'icv' => $icv->next($company), 'invoice_type' => $data['invoice_type'], 'invoice_subtype' => $data['invoice_subtype'], 'invoice_scope' => $data['invoice_scope'] ?? 'local', 'payment_type' => $data['payment_type'] ?? 'receivable', 'taxpayer_type' => $data['taxpayer_type'] ?? 'income', 'issue_date' => now()->toDateString(), 'issue_time' => now()->format('H:i:s'), 'currency_code' => $data['currency_code'], 'exchange_rate' => '1', 'supplier_id' => $company->id, 'customer_id' => $data['customer_id'] ?? null, 'status' => 'DRAFT']));
