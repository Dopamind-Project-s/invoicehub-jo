...
### 2026-06-19 — Official Spatie Permission and Breeze verification pass
- **Changed files:** `tests/TestCase.php` disables Vite during feature tests so Breeze-authenticated pages can be rendered in the test environment without requiring a built frontend manifest; `docs/phase-1-todo.md` records this verification status.
- **Commands executed:** `composer install`; `php artisan optimize:clear`; `php artisan migrate --force`; `php artisan list permission --raw`; `php artisan permission:cache-reset`; `php artisan test`; `php artisan test tests/Feature/CompanyWorkspaceFoundationTest.php --stop-on-failure`; `php artisan test tests/Feature/CompanyWorkspaceFoundationTest.php --debug`; vendor/class checks for `vendor/spatie/laravel-permission/src/Models/Role.php`, `Spatie\Permission\Models\Role`, and `Laravel\Breeze\BreezeServiceProvider`.
- **Verification status:** blocked by dependency installation. Composer attempted to install official `laravel/breeze`, `spatie/laravel-package-tools`, and `spatie/laravel-permission`, but GitHub dist/source downloads were denied by the environment (`CONNECT tunnel failed, response 403`). The resulting vendor package directories exist but do not contain loadable official classes, and `php -r "require 'vendor/autoload.php'; class_exists(...)"` returns false for both Spatie Permission Role and Breeze service provider.
- **Spatie teams findings:** `config/permission.php` is configured for teams and uses `company_id` as the team key. Company workspace code calls `setPermissionsTeamId($company->id)` before role/permission assignment and checks; route middleware still applies `permission.team` before Spatie `permission:*` checks.
- **Super Admin/company isolation findings:** existing test coverage is present for Super Admin bypass, company role isolation, company permission middleware, role assignment, settings, activity, and dashboard access in `tests/Feature/CompanyWorkspaceFoundationTest.php`; execution is currently blocked once official Spatie classes are missing from vendor.
- **Breeze findings:** Breeze auth views/routes are present, but the first full-suite run surfaced a missing Vite manifest for Breeze-auth-rendered pages. `tests/TestCase.php` now calls `withoutVite()` for feature tests to keep auth/dashboard behavior testable without a frontend build artifact.
- **Old local shim status:** no `app/Support/SpatiePermission` files remain in the working tree, and application code now references the official `Spatie\Permission` namespaces. Documentation still preserves the historical note that a shim existed in the prior step.
- **Tests passed/failed:** `php artisan optimize:clear` passed; `php artisan migrate --force` passed; `php artisan permission:cache-reset` is unavailable because the official package command provider is not loadable; `php artisan test` failed due missing/empty official vendor packages after composer was blocked; targeted workspace tests pass only the config/database seed check before failing on missing `Spatie\Permission\Models\Role`.
- **Remaining risks:** official Spatie Permission and Breeze integration cannot be fully verified until `composer install` can fetch/populate package source code or the correct vendor artifact is restored; current `vendor/spatie/laravel-permission` and `vendor/laravel/breeze` directories are empty/non-functional in this environment.
- **Recommended next execution step:** restore dependency installation access or provide a populated vendor/cache artifact for the official packages, then rerun `composer install`, `php artisan optimize:clear`, `php artisan migrate --force`, `php artisan permission:cache-reset`, and the full test suite before starting any new business module.
