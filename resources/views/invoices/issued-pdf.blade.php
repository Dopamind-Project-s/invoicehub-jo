...
<head>
<meta charset="utf-8">
<link rel="stylesheet" href="{{ public_path('css/invoice-pdf.css') }}">
</head>
<body>
@if($invoice->status !== 'ACCEPTED')<div class="watermark">غير مَفْوترة</div>@endif
<table class="header"><tr><td><div class="title">{{ $invoice->status === 'ACCEPTED' ? 'فاتورة مَفْوترة' : 'مسودة فاتورة' }}</div><div class="subtitle">دخل ذمم / محلي / JOD</div></td><td class="ltr text-left">{{ $invoice->invoice_number }}</td></tr></table>
