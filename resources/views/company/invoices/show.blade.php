...
@php
    ...
    $canJofotara = $hasSubmitFeature && $hasCredentials && ($company->is_active ?? true) && auth()->user()?->can('invoices.submit') && $invoice->status === 'ready' && ! in_array($invoice->jofotara_status, ['ACCEPTED', 'SUBMITTED'], true);
    $warnings = [];
...
@if($invoice->status === 'ready' && ! $canJofotara)
