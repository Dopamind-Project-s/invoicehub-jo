...
<div class="invoice-show theme-{{ $templateSlug }}">
    <div class="invoice-summary-hero d-flex flex-column flex-xl-row justify-content-between gap-3 align-items-xl-center">
        ...
        <div class="invoice-actions d-flex flex-wrap gap-2">
            ...
            <form method="post" action="{{ route('company.invoices.shares.store', [$company, $invoice]) }}">@csrf<input type="hidden" name="channel" value="link"><button class="btn btn-outline-light invoice-action-btn">🔗 مشاركة</button></form>
            <form method="post" action="{{ route('company.invoices.shares.store', [$company, $invoice]) }}">@csrf<input type="hidden" name="channel" value="whatsapp"><button class="btn btn-outline-light invoice-action-btn">🟢 واتساب</button></form>
