...
<div class="category-form" dir="rtl">
    ...
    <div class="category-form-body">
        ...
        <section class="category-section"><div class="category-section-title">1. البيانات الأساسية</div><div class="row g-3">
            ...
            <div class="col-md-6"><label class="form-label">الاسم الإنجليزي</label><input name="name_en" class="form-control" placeholder="Example: Main meals" value="{{ old('name_en', $category->name_en) }}">@error('name_en')<div class="text-danger small">{{ $message }}</div>@enderror</div>
            <div class="col-md-6"><label class="form-label">الكود</label><input name="code" class="form-control" placeholder="مثال: MAIN-MEALS" value="{{ old('code', $category->code) }}" required><div class="form-text">كود داخلي مختصر وفريد داخل المنشأة.</div>@error('code')<div class="text-danger small">{{ $message }}</div>@enderror</div>
