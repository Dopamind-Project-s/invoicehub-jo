...
<div class="row g-3" dir="rtl">
    <div class="col-md-6"><label class="form-label">الاسم عربي</label><input name="name_ar" class="form-control" value="{{ old('name_ar', $company->name_ar ?: $company->legal_name_ar) }}" required></div>
    <div class="col-md-6"><label class="form-label">الاسم إنجليزي</label><input name="name_en" class="form-control" value="{{ old('name_en', $company->name_en ?: $company->legal_name_en) }}"></div>
    ...
    <div class="col-md-4"><label class="form-label">الشعار</label><input name="logo" type="file" accept="image/*" class="form-control"></div>
    ...
    @foreach($features as $feature)
        <div class="col-md-4">
            <label class="form-check border rounded p-3 h-100">
                ...
                <span class="form-check-label"><strong>{{ $feature->code }}</strong><br><small class="text-muted">{{ $feature->name }}</small></span>
            </label>
        </div>
    @endforeach
</div>
